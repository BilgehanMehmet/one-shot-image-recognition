...
class Loader(object):
	...
	def __init__(self,n_examples,mode,path='/Users/mehmetbezcioglu/Downloads/images_background',normalise=False):
			...
			for file in files:
				if ".png" in file or ".pgm" in file:
					...
					if ((mode == "train_split") and (("_19" in file) or ("_20" in file))):
							...
							continue
					if (mode == "orl_split" and file.split(".")[0] in self.validation_images):
							self.X_validation.append(self.import_and_resize_image(os.path.join(subdirectory,file)))
							continue
		...
		self.X = self._normalise(self.X,n_examples = n_examples-len(self.validation_images),normalise= normalise) if mode == "train_split" or mode == "orl_split" else self._normalise(self.X,n_examples = n_examples,normalise= normalise)
		self.X_validation = self._normalise(self.X_validation,n_examples = len(self.validation_images),normalise= normalise) if mode == "train_split" or mode == "orl_split"  else None
