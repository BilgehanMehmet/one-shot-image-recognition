...
class Loader(object):
	...
	def __init__(self,n_examples,mode,path='/Users/mehmetbezcioglu/Downloads/images_background',normalise=False):
		...
		# self.X = np.asarray(self.X)[:-10]
		# file_.close()
		print(len(self.X))
		self.X = self.__normalise(self.X,n_examples = n_examples-2,normalise= normalise) if mode == "train_split" else self.__normalise(self.X,n_examples = n_examples,normalise= normalise)
		self.X_validation = self.__normalise(self.X_validation,n_examples = 2,normalise= normalise) if mode == "train_split" else None
		# remainder = len(self.X) % n_examples
		# if remainder:
		# 	self.X = np.asarray(self.X)[:-remainder]
		# else:
		# 	self.X = np.asarray(self.X)
		# self.X = self.X.reshape(-1,n_examples,self.X.shape[1],self.X.shape[2],1)
		# print(self.X.shape)
		# if normalise:
			# self.X = self.X.reshape(-1,105*105)
			# mu = np.mean(self.X,axis=0).reshape(1,11025)
			# # std = np.std(self.X,axis=0).reshape(1,11025)
			# var = np.var(self.X,axis=0).reshape(1,11025)
			# self.X = np.subtract(self.X,mu)
			# self.X /= (var + 1e-100)
			# self.X = self.X.reshape(-1,n_examples,105,105,1)



	@staticmethod
	def __normalise(X,n_examples,normalise):
